...
#define _DUTY_NEU 1510  // servo neutral position (90 degree)
...
void loop()
{
  ...
  dist_ema = dist_raw *_EMA_ALPHA + (1-_EMA_ALPHA) * (dist_ema_prev);
  dist_ema_prev = dist_ema; // Put EMA filter code here
      
  //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
  duty = (dist_ema - _DIST_MIN) * 13.2 + _DUTY_MIN; // Put map() equivalent code here
